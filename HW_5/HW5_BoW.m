...
% Last modified: 16-Oct-2018    
    ...
    methods (Static)
        function main()
            ...
            [output,acc,~] = svmpredict(zeros(t,1), tstDK, model);
%             C = confusionmat(testLabel, output)                   %# confusion matrix
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Write code for RBS kernel implementation by Rohit Bhal         %
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

        function [trainK, testK] = cmpRBSKernel(trainD, testD, gamma, TestRequired)
            [n, d] = size(trainD);
            [t, f] = size(testD);
            p = 0;
            trainK = zeros(n,n);
            for i=1:n
                trainK(i,i) = 1;
                for j=1:i-1
                    trainK(i,j)=exp(-gamma*((norm(trainD(i,:)-trainD(j,:)).^2)));
                    trainK(j,i)=trainK(i,j);
                end
            end
            trainK = [(1:n).', trainK];
            testK = zeros(t,n);
            if TestRequired
                for i=1:t
                    testK(i,i) = 1;
                    for j=1:i-1
                        testK(i,j)=exp(-gamma*(norm(testD(i,:)-trainD(j,:)).^2));
                        testK(j,i)=testK(i,j);
                    end
                end
            end
        ...
        function bowCs = learnDictionary(scales, normH, normW)
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%             size(randWins)
            [bowCs, ~] = kmeans(randWins.', [], 1000, false);
            bowCs = bowCs.';
